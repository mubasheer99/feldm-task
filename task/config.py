...
task1_most_revenue_user = r'''with revenue as
...
task2_most_revenue_day = r'''select t.datetime revenue_date,sum(revenue) revenue from devices d,Transactions t
                            where d.id=t.device_type and d.device_name='Mobile Phone'
                            group by t.datetime order by 2 desc'''

task3_combine_tables = {'devices': 'select id device_id,device_name from devices', 'transaction': 'select * from Transactions'}

task5_create_insert_table = {'create': 'CREATE TABLE IF NOT EXISTS Devices(id     INTEGER,device_name TEXT,PRIMARY KEY(id))', 'insert': '''INSERT INTO Devices ("id","device_name") VALUES (1,'Desktop'),(2,'Tablet'),(3,'Mo bile Phone'),(4,'Unknown') ON CONFLICT (id) DO NOTHING;''', 'select': 'select * from devices'}
